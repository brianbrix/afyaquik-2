...
export async function bulkUpsertTriageEntries(queueItemId: number, entries: Array<{ id?: number; title: string; details: string }>): Promise<TriageEntryDto[]> {
  const res = await apiClient.post(`/queue/${queueItemId}/triage-entries/bulk`, { entries });
  return res.data?.data ?? [];
}

export async function bulkDeleteTriageEntries(queueItemId: number, ids: number[]): Promise<void> {
  await apiClient.post(`/queue/${queueItemId}/triage-entries/bulk-delete`, { ids });
}
