...
import { SHIFT_STATUS_LABELS, type ShiftStatus, type ShiftSwapApprovalPayload, type ShiftSwapRequestPayload } from "../../../types/scheduling";
...
const BASE_FILTER_VALUES: FilterBarValues = { searchTerm: "", status: "", departmentId: "", roleId: "", rangeStart: "", rangeEnd: "", restrictToRole: true };
...
function ShiftCreateModal({ show, onHide, onSubmit, isSubmitting, error, defaultRole, roles, departments, shiftTypes, shiftTypesError }: ShiftCreateModalProps) {
  ...
  return (<FormModal show={show} title="Create shift" submitLabel="Create shift" onHide={onHide} onSubmit={onSubmit} isSubmitting={isSubmitting} error={error} size="lg">
    <Row className="g-3">
      ...
      <Col md={6}><Form.Group controlId="cRole"><Form.Label className="fw-semibold">Role</Form.Label><Form.Select name="roleId" defaultValue={defaultRole}>{roles.map(r=> <option key={r.id} value={r.id}>{r.displayName}</option>)}</Form.Select></Form.Group></Col>
      <Col md={6}><Form.Group controlId="cDept"><Form.Label className="fw-semibold">Department</Form.Label><Form.Select name="departmentId" required defaultValue=""> <option value="" disabled>Select…</option> {departments.map(d=> <option key={d.id} value={d.id}>{d.displayName}</option>)} </Form.Select></Form.Group></Col>
      <Col md={6}>
        <Form.Group controlId="cType">
          <Form.Label className="fw-semibold">Shift type</Form.Label>
          <Form.Select name="shiftType" required disabled={!!shiftTypesError || shiftTypes.length === 0}>
          ...
          {shiftTypesError && <div className="small text-danger">Failed to load shift types</div>}
        </Form.Group>
      </Col>
...
type ShiftEditModalProps = { show: boolean; onHide: () => void; onSubmit: (e: FormEvent<HTMLFormElement>) => void; isSubmitting: boolean; error?: unknown; shift: StaffShift | null; roles: RoleDefinition[]; departments: DepartmentDefinition[]; shiftTypes: ShiftTypeModel[]; shiftTypesError: unknown };
function ShiftEditModal({ show, onHide, onSubmit, isSubmitting, error, shift, roles, departments, shiftTypes, shiftTypesError }: ShiftEditModalProps) {
  if(!shift) return null;
  return (<FormModal show={show} title="Update shift" submitLabel="Save changes" onHide={onHide} onSubmit={onSubmit} isSubmitting={isSubmitting} error={error} size="lg">
    <Row className="g-3">
      <Col md={6}>
        <Form.Group controlId="eType">
          <Form.Label className="fw-semibold">Shift type</Form.Label>
          <Form.Select name="shiftType" defaultValue={shift?.shiftType} required disabled={!!shiftTypesError || shiftTypes.length === 0}>
            <option value="" disabled>Select…</option>
            {shiftTypes.map((t: ShiftTypeModel) => (
              <option key={t.id} value={t.id}>
                {t.name} ({t.startTime}–{t.endTime})
      ...
      <Col md={6}><Form.Group controlId="eRole"><Form.Label className="fw-semibold">Role</Form.Label><Form.Select name="roleId" defaultValue={shift.roleId}>{roles.map(r=> <option key={r.id} value={r.id}>{r.displayName}</option>)}</Form.Select></Form.Group></Col>
      <Col md={6}><Form.Group controlId="eDept"><Form.Label className="fw-semibold">Department</Form.Label><Form.Select name="departmentId" defaultValue={shift.departmentId}>{departments.map(d=> <option key={d.id} value={d.id}>{d.displayName}</option>)}</Form.Select></Form.Group></Col>
...
function ShiftSwapRequestModal({ show, onHide, onSubmit, isSubmitting, error, shift }: ShiftSwapRequestModalProps) { if(!shift) return null; return (<FormModal show={show} title="Request shift swap" submitLabel="Submit request" onHide={onHide} onSubmit={onSubmit} isSubmitting={isSubmitting} error={error}>
  {/* Shift type label will be updated to use dynamic shift types */}
...
function mapFilters(values: FilterBarValues, activeRole?: number): StaffShiftFilters {
  ...
  if (restrict && activeRole) f.roleId = activeRole;
  else if (!restrict && role) f.roleId = role;
...
export function SchedulingCalendarPage(){
  ...
  const getShiftType = (id: number) => shiftTypes.find(t => t.id === id);
  ...
  const filtered = useMemo(()=> !search ? shifts : shifts.filter(s => [s.staffDisplayName,s.departmentId,s.roleKey,getShiftType(s.shiftType)?.name || s.shiftType,s.notes??"",s.handoverNotes??""].filter(Boolean).map(v=>v!.toLowerCase()).some(v=>v.includes(search))), [shifts, search, shiftTypes]);
  ...
  const onCreate = (e: FormEvent<HTMLFormElement>)=>{ e.preventDefault(); const fd=new FormData(e.currentTarget); const payload = { staffUserId: Number(fd.get('staffUserId')), roleKey: (fd.get('roleKey')?.toString().trim()|| activeRole || ""), departmentId: fd.get('departmentId')?.toString().trim()||"", shiftType: Number(fd.get('shiftType')), startsAt: localDateTimeToIso(fd.get('startsAt')?.toString()||""), endsAt: localDateTimeToIso(fd.get('endsAt')?.toString()||""), notes: extractNotes(fd,'notes') }; if(!payload.roleKey) return; createShift.mutate(payload as any,{ onSuccess:()=>{ e.currentTarget.reset(); close(); }}); };
  const onEdit = (e: FormEvent<HTMLFormElement>)=>{ e.preventDefault(); if(!selected) return; const fd=new FormData(e.currentTarget); const payload = { staffUserId: Number(fd.get('staffUserId')), roleKey: fd.get('roleKey')?.toString().trim()||undefined, departmentId: fd.get('departmentId')?.toString().trim()||undefined, shiftType: fd.get('shiftType') ? Number(fd.get('shiftType')) : undefined, status: fd.get('status') as ShiftStatus, startsAt: localDateTimeToIso(fd.get('startsAt')?.toString()||""), endsAt: localDateTimeToIso(fd.get('endsAt')?.toString()||""), notes: extractNotes(fd,'notes'), handoverNotes: extractNotes(fd,'handoverNotes') }; updateShift.mutate({ shiftId: selected.id, payload: payload as any }, { onSuccess: close }); };
  ...
  const filterFields: FilterFieldConfig[] = [
    ...
    { key: 'shiftType', type: 'select', label: 'Shift Type', placeholder: 'All', col: 2, options: shiftTypes.map(t => ({ value: t.id, label: `${t.name} (${t.startTime}–${t.endTime})` })) },
    { key: 'departmentId', type: 'select', label: 'Department', placeholder: 'All', col: 2, options: (deptsQuery.data ?? []).map(d => ({ value: d.id, label: d.displayName })) },
    { key: 'roleId', type: 'select', label: 'Role', placeholder: 'All', col: 2, options: (rolesQuery.data ?? []).map(r => ({ value: r.id, label: r.displayName })) },
                  ...
                  {(() => {
                    ...
                    return st ? (
                      <>
                        ...
                        <div className="text-muted small">{st.startTime}–{st.endTime}</div>
                        <div className="text-muted small">{dateFormatter.format(new Date(s.startsAt))} · {formatTimeRange(s)}</div>
                      </>
                    ) : (
                      <div className="text-muted small">Unknown shift type</div>
                    );
                  })()}
                </td>
  ...
  <ShiftCreateModal show={modal==='create'} onHide={close} onSubmit={onCreate} isSubmitting={createShift.isPending} error={createShift.isError? createShift.error: undefined} defaultRole={activeRole||''} roles={rolesQuery.data ?? []} departments={deptsQuery.data ?? []} shiftTypes={shiftTypes} shiftTypesError={shiftTypesError} />
  <ShiftEditModal show={modal==='edit'} onHide={close} onSubmit={onEdit} isSubmitting={updateShift.isPending} error={updateShift.isError? updateShift.error: undefined} shift={selected} roles={rolesQuery.data ?? []} departments={deptsQuery.data ?? []} shiftTypes={shiftTypes} shiftTypesError={shiftTypesError} />
