...
public interface StaffShiftRepository extends TenantAwareRepository<StaffShift, Long>, JpaSpecificationExecutor<StaffShift> {
    ...
    default boolean existsOverlappingShift(String tenantId, Long staffUserId, LocalDateTime startsAt, LocalDateTime endsAt, Long excludeId) {
    ...
    default List<StaffShift> findPendingCheckIn(String tenantId, Long staffUserId, com.afyaquik.hms.scheduling.domain.ShiftStatus status, LocalDateTime before) {
        var spec = (org.springframework.data.jpa.domain.Specification<StaffShift>) (root, query, cb) -> {
            var p = cb.conjunction();
            p = cb.and(p, cb.equal(root.get("tenantId"), tenantId));
            p = cb.and(p, cb.equal(root.get("staffUser").get("id"), staffUserId));
    ...
    default List<StaffShift> findPendingCheckOut(String tenantId, Long staffUserId, List<com.afyaquik.hms.scheduling.domain.ShiftStatus> statuses, LocalDateTime before) {
        var spec = (org.springframework.data.jpa.domain.Specification<StaffShift>) (root, query, cb) -> {
            var p = cb.conjunction();
            p = cb.and(p, cb.equal(root.get("tenantId"), tenantId));
            p = cb.and(p, cb.equal(root.get("staffUser").get("id"), staffUserId));
    ...
    default boolean existsOverlappingShiftForCurrentTenant(Long staffUserId, LocalDateTime startsAt, LocalDateTime endsAt, Long excludeId) {
        String tenantId = TenantHeaderInterceptor.getCurrentTenant();
        return existsOverlappingShift(tenantId, staffUserId, startsAt, endsAt, excludeId);
    }
    
    default List<StaffShift> findPendingCheckInForCurrentTenant(Long staffUserId, ShiftStatus status, LocalDateTime before) {
        String tenantId = TenantHeaderInterceptor.getCurrentTenant();
        return findPendingCheckIn(tenantId, staffUserId, status, before);
    }
    
    default List<StaffShift> findPendingCheckOutForCurrentTenant(Long staffUserId, List<ShiftStatus> statuses, LocalDateTime before) {
        String tenantId = TenantHeaderInterceptor.getCurrentTenant();
    ...
    // Dashboard methods
    List<StaffShift> findByStaffUserIdAndStartsAtBetween(Long staffUserId, LocalDateTime start, LocalDateTime end);
