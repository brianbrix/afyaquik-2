...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
    ...
    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.deleted = false")
    ...
    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.quantityInStock <= i.minimumStockLevel AND i.deleted = false")
    ...
    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.reorderPoint IS NOT NULL AND i.quantityInStock <= i.reorderPoint AND i.deleted = false")
    List<Inventory> findNeedingReorderByTenantId(@Param("tenantId") String tenantId);

    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.expiryDate IS NOT NULL AND i.expiryDate <= :expiryDate AND i.deleted = false")
    ...
    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.expiryDate IS NOT NULL AND i.expiryDate < CURRENT_DATE AND i.deleted = false")
    ...
    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.supplier = :supplier AND i.deleted = false")
    List<Inventory> findByTenantIdAndSupplier(@Param("tenantId") String tenantId, @Param("supplier") String supplier);

    @Query("SELECT i FROM Inventory i WHERE i.tenantId = :tenantId AND i.location = :location AND i.deleted = false")
    List<Inventory> findByTenantIdAndLocation(@Param("tenantId") String tenantId, @Param("location") String location);

    @Query("SELECT DISTINCT i.supplier FROM Inventory i WHERE i.tenantId = :tenantId AND i.supplier IS NOT NULL AND i.deleted = false ORDER BY i.supplier")
    List<String> findDistinctSuppliersByTenantId(@Param("tenantId") String tenantId);

    @Query("SELECT DISTINCT i.location FROM Inventory i WHERE i.tenantId = :tenantId AND i.location IS NOT NULL AND i.deleted = false ORDER BY i.location")
